...
public:
   ...
   * AddTBlock
   * 
   * \param color The T block color
   * \param programID The Id of the shader
   * \return T block game objects, 4 blocks made a tetromino.
   */
  static vector<GameObject*> AddTBlock(float scale, vec3 color, Material* material)
  {
    ...
    GameObject* g = new GameObject(t, r, "TBlock");
    g->shader = new PhongShader(r, Scene::CurrentScene()->GetLigthSources());
    gameObjects.push_back(g);


    t = new Transform(vec3(-15, 185, 1), vec3(0), vec3(scale));
